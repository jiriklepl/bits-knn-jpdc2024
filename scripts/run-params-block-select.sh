#!/bin/bash
#SBATCH -o data/params-block-select-%N-%j.csv
#SBATCH -e data/params-block-select-%N-%j.err
#SBATCH --gpus 1
#SBATCH -p gpu-short
#SBATCH --time=2:00:00
#SBATCH --mem=0
...
for n in 128 512 1024; do
    ...
    for k in 32 64 128 256 512 1024 2048; do
        for thread_queue in 2 3 4 5 6 7 8 9 10; do
            "$knn" -r "$repeat_count" -n "$n"k -q "$q"k -k "$k" --seed 24 --block-size "$block_size" --items-per-thread "$thread_queue" -a block-select-tunable
        done
