...
Slurm job IDs replace the timestamps used by local runs; see [data-format.md](data-format.md) for result file names and the CSV format.

For the three application benchmarks, use the `applications-build` and `applications-run` actions described in [applications.md](applications.md).
