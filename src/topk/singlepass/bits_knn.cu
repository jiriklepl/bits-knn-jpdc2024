...
struct bits
{
    ...
    {
        run_bits_kernel<PREFETCH, COALESCED_WRITE, ADD_NORMS, BLOCK_SIZE, BATCH_SIZE, K>(
    ...
    void run(std::size_t k)
    {
        if (k > 0 && k <= 16)
        {
            run<PREFETCH, COALESCED_WRITE, ADD_NORMS, BLOCK_SIZE, BATCH_SIZE, 16>(k);
        }
        else if (k > 0 && k <= 32)
        {
            run<PREFETCH, COALESCED_WRITE, ADD_NORMS, BLOCK_SIZE, BATCH_SIZE, 32>(k);
        }
        else if (k > 0 && k <= 64)
        {
            run<PREFETCH, COALESCED_WRITE, ADD_NORMS, BLOCK_SIZE, BATCH_SIZE, 64>(k);
        }
        else if (k > 0 && k <= 128)
        {
            run<PREFETCH, COALESCED_WRITE, ADD_NORMS, BLOCK_SIZE, BATCH_SIZE, 128>(k);
        }
        else if (k > 0 && k <= 256)
        {
            run<PREFETCH, COALESCED_WRITE, ADD_NORMS, BLOCK_SIZE, BATCH_SIZE, 256>(k);
        }
        else if (k > 0 && k <= 512)
        {
            run<PREFETCH, COALESCED_WRITE, ADD_NORMS, BLOCK_SIZE, BATCH_SIZE, 512>(k);
        }
        else if (k > 0 && k <= 1024)
        {
            run<PREFETCH, COALESCED_WRITE, ADD_NORMS, BLOCK_SIZE, BATCH_SIZE, 1024>(k);
        }
        else if (k > 0 && k <= 2048)
        {
            run<PREFETCH, COALESCED_WRITE, ADD_NORMS, BLOCK_SIZE, BATCH_SIZE, 2048>(k);
        }
        else
        {
    ...
    void run(std::size_t batch_size, std::size_t k)
    {
        if (batch_size == 1)
        {
            run<PREFETCH, COALESCED_WRITE, ADD_NORMS, BLOCK_SIZE, 1>(k);
        }
        else if (batch_size == 2)
        {
            run<PREFETCH, COALESCED_WRITE, ADD_NORMS, BLOCK_SIZE, 2>(k);
        }
        else if (batch_size == 3)
        {
            run<PREFETCH, COALESCED_WRITE, ADD_NORMS, BLOCK_SIZE, 3>(k);
        }
        else if (batch_size == 4)
        {
            run<PREFETCH, COALESCED_WRITE, ADD_NORMS, BLOCK_SIZE, 4>(k);
        }
        else if (batch_size == 5)
        {
            run<PREFETCH, COALESCED_WRITE, ADD_NORMS, BLOCK_SIZE, 5>(k);
        }
        else if (batch_size == 6)
        {
            run<PREFETCH, COALESCED_WRITE, ADD_NORMS, BLOCK_SIZE, 6>(k);
        }
        else if (batch_size == 7)
        {
            run<PREFETCH, COALESCED_WRITE, ADD_NORMS, BLOCK_SIZE, 7>(k);
        }
        else if (batch_size == 8)
        {
            run<PREFETCH, COALESCED_WRITE, ADD_NORMS, BLOCK_SIZE, 8>(k);
        }
        else if (batch_size == 9)
        {
            run<PREFETCH, COALESCED_WRITE, ADD_NORMS, BLOCK_SIZE, 9>(k);
        }
        else if (batch_size == 10)
        {
            run<PREFETCH, COALESCED_WRITE, ADD_NORMS, BLOCK_SIZE, 10>(k);
        }
        else if (batch_size == 11)
        {
            run<PREFETCH, COALESCED_WRITE, ADD_NORMS, BLOCK_SIZE, 11>(k);
        }
        else if (batch_size == 12)
        {
            run<PREFETCH, COALESCED_WRITE, ADD_NORMS, BLOCK_SIZE, 12>(k);
        }
        else if (batch_size == 13)
        {
            run<PREFETCH, COALESCED_WRITE, ADD_NORMS, BLOCK_SIZE, 13>(k);
        }
        else if (batch_size == 14)
        {
            run<PREFETCH, COALESCED_WRITE, ADD_NORMS, BLOCK_SIZE, 14>(k);
        }
        else if (batch_size == 15)
        {
            run<PREFETCH, COALESCED_WRITE, ADD_NORMS, BLOCK_SIZE, 15>(k);
        }
        else if (batch_size == 16)
        {
            run<PREFETCH, COALESCED_WRITE, ADD_NORMS, BLOCK_SIZE, 16>(k);
        }
        else
        {
    ...
    template <bool PREFETCH, bool COALESCED_WRITE, bool ADD_NORMS>
    void run(std::size_t block_size, std::size_t batch_size, std::size_t k)
    {
        if (block_size == 128)
        {
            run<PREFETCH, COALESCED_WRITE, ADD_NORMS, 128>(batch_size, k);
        }
        else if (block_size == 256)
        {
            run<PREFETCH, COALESCED_WRITE, ADD_NORMS, 256>(batch_size, k);
        }
        else if (block_size == 512)
        {
            run<PREFETCH, COALESCED_WRITE, ADD_NORMS, 512>(batch_size, k);
        }
        else
        {
...
void bits_knn::selection()
{
    ...
    kernel.run<PREFETCH, COALESCED_WRITE, ADD_NORMS>(block_size, batch_size, k());
